...
export class ClientView extends LitElement {
  ...
  private async _togglePolicyEnabled(p:Policy){
    ...
    this.state={...this.state,policies:this.state.policies.map(x=>x.id===p.id?{...x,enabled:newEnabled}:x)};
    this.onStateChanged?.();
  }
