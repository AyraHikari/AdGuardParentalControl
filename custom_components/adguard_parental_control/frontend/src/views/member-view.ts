...
export class MemberView extends LitElement {
  ...
  private _isBlocked(q: QueryLogEntry) {
    ...
    return /filtered\/(blocked|blacklist|safebrowsing|parental|safesearch|service)/.test(r) ||
           /\bblocked\b/.test(r) || /\bblacklist\b/.test(r);
  }
  ...
  private async _addClient(name: string) { if (!name || this.member.client_names.includes(name)) return; const updated = { ...this.member, client_names: [...this.member.client_names, name] }; await this.hass.callWS({ type: "adguard_pc/members/update", member: updated }); this.onStateChanged?.(); this._loadQueryLog(); }
  private async _removeClient(name: string) { const updated = { ...this.member, client_names: this.member.client_names.filter((c) => c !== name) }; await this.hass.callWS({ type: "adguard_pc/members/update", member: updated }); this.onStateChanged?.(); this._loadQueryLog(); }
  private async _addException() { if (!this._newException.trim()) return; const updated = { ...this.member, exceptions: [...this.member.exceptions, this._newException.trim()] }; await this.hass.callWS({ type: "adguard_pc/members/update", member: updated }); this._newException = ""; this.onStateChanged?.(); }
  private async _removeException(ex: string) { const updated = { ...this.member, exceptions: this.member.exceptions.filter((e) => e !== ex) }; await this.hass.callWS({ type: "adguard_pc/members/update", member: updated }); this.onStateChanged?.(); }
