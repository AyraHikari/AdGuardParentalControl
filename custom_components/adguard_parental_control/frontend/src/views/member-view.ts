  ...
  render() {
    ...
    return html`
      ...
      ${this._showDeleteConfirm ? html`
        <ha-dialog open @closed=${this._handleDeleteDialog}><p>Delete member "${this.member.name}"?</p><mwc-button slot="secondaryAction" @click=${() => this._showDeleteConfirm = false}>Cancel</mwc-button><mwc-button slot="primaryAction" @click=${this._deleteMember}>Delete</mwc-button></ha-dialog>
      ` : ""}
    `;
  }
